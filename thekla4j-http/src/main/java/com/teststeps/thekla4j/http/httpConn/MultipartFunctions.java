...
public class MultipartFunctions {
  ...
  protected static final Function6<Part, Integer, Integer, String, String, StringWriter, Try<Void>>
    appendPart = (part, index, length, boundary, line, writer) -> Try.of(() -> {
      writer
      .append("Content-Disposition: form-data; name=\"").append(part.name()).append("\"")
      .append(line)
      .append("Content-Type: ").append(part.contentType().asString())
      .append(line)
      .append(line)
      .append(part.value())
      .append(line)
  ...
  protected static final Function6<List<FilePart>, String, String, PrintWriter, OutputStream, StringWriter, Try<Void>> appendFileParts =
    (fileParts, boundary, line, printWriter, outputStream, logWriter) -> fileParts
      .zipWithIndex()
      .map(
        filePartTuple -> MultipartFunctions.appendFilePart.apply(filePartTuple._1, filePartTuple._2, fileParts.length(), boundary, line, printWriter,
  ...
  protected static final Function8<FilePart, Integer, Integer, String, String, PrintWriter, OutputStream, StringWriter, Try<Void>> appendFilePart =
    (filePart, index, length, boundary, line, printWriter, outputStream, logWriter) -> {
      ...
      sw.append("Content-Disposition: form-data; name=\"" + filePart.fieldName() + "\"; filename=\"" + filePart.file().getName() + "\"").append(line);
