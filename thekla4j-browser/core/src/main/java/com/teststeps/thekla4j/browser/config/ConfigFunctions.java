...
public class ConfigFunctions {
  ...
  /**
   * Load the BrowserConfig from the configuration file in case the file cant be loaded an empty BrowserConfig is
  ...
  browserConfigList.map(bcl -> bcl.browserConfigs()
      .get(BROWSER_CONFIG.optionValue().getOrElse(bcl.defaultConfig()))
      .getOrElseThrow(() -> new IllegalArgumentException(
                                                         """
                                                             ...
                                                             .replace("$$CONFIG_FILE$$", browserConfigList.map(YAML::jStringify)
