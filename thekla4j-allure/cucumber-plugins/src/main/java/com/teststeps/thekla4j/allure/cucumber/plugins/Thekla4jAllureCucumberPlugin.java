...
/**
 * Thekla4j Allure plugin for Cucumber JVM 7.
 * Handles ActivityError as FAILED (not BROKEN) and supports thekla4j-specific tags.
 */
@SuppressWarnings(
  {"ClassDataAbstractionCoupling", "ClassFanOutComplexity", "PMD.ExcessiveImports",
  }
)
public class Thekla4jAllureCucumberPlugin implements ConcurrentEventListener {
  ...
  private void handleTestCaseStarted(final TestCaseStarted event) {
    ...
    final String testId = labelBuilder.getTestId();
    final TestResult result = new TestResult()
        .setUuid(testCaseUuid)
        .setTestCaseId(testId.isEmpty() ? null : testId)
        .setFullName(fullName)
        .setName(testId.isEmpty() ? name : name + " (testId: " + testId + ")")
  ...
  private void handleTestCaseFinished(final TestCaseFinished event) {
    ...
    final StatusDetails statusDetails = buildStatusDetails(result.getError());
    ...
    lifecycle.updateTestCase(uuid, testResult -> {
      ...
      testResult.setHistoryId(getHistoryId(testCase, testResult));
  ...
  private void handleStartPickleStep(
                                     final TestCase testCase, final PickleStepTestStep pickleStep
  ) {
    final String uuid = testCase.getId().toString();
  ...
  private void handleStartStepHook(
                                   final TestCase testCase, final HookTestStep hook
  ) {
    final String uuid = testCase.getId().toString();
    final StepResult stepResult = new StepResult()
  ...
  private void handleStartFixtureHook(
                                      final TestCase testCase, final HookTestStep hook
  ) {
    final String uuid = testCase.getId().toString();
  ...
  private String getHistoryId(final TestCase testCase, final TestResult result) {
    ...
    String parameterList = result.getParameters()
        .stream()
        .map(p -> p.getName() + COLON + p.getValue())
        .collect(Collectors.joining(","));
  ...
  private Status translateTestCaseStatus(final Result testCaseResult) {
    switch (testCaseResult.getStatus()) {
      case FAILED:
        final Throwable error = testCaseResult.getError();
        if (error instanceof ActivityError) {
          return Status.FAILED;
        }
        return getStatus(error).orElse(Status.FAILED);
      case PASSED:
  ...
  private StatusDetails buildStatusDetails(final Throwable error) {
    if (error instanceof ActivityError) {
      return new StatusDetails()
          .setMessage(error.getMessage())
          .setTrace(stackTraceOf(error));
    }
    return getStatusDetails(error).orElseGet(StatusDetails::new);
  }
  ...
  private List<Parameter> getExamplesAsParameters(
    ...
    final Optional<Examples> maybeExample =
        scenario.getExamples()
            .stream()
            .filter(example -> example.getTableBody()
                .stream()
    ...
    final Optional<TableRow> maybeRow = examples.getTableBody()
        .stream()
    ...
    return IntStream.range(0, size)
        .mapToObj(index -> {
          final String name = Objects.nonNull(headerNames) ? headerNames.get(index) : "arg" + index;
  ...
  private void handleStopHookStep(
                                  final Result eventResult, final HookTestStep hook
  ) {
  ...
  private void handleStopStep(
                              final TestCase testCase, final Result eventResult, final UUID stepId
  ) {
    final Feature feature = testSources.getFeature(testCase.getUri());

    final Status stepStatus = translateTestCaseStatus(eventResult);

    final StatusDetails statusDetails = eventResult.getStatus() == io.cucumber.plugin.event.Status.UNDEFINED ? new StatusDetails().setMessage(
      "Undefined Step. Please add step definition") : getStatusDetails(eventResult.getError())
