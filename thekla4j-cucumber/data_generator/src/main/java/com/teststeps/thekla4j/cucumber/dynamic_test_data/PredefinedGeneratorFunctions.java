    ...
    if (data.keySet().contains("default")) {
      tryLength = data.get("default")
          ...
          .mapTry(i -> Try.of(() -> Integer.parseInt(i))
              .getOrElseThrow(x -> new IllegalArgumentException("Generate random string failed: default value must be an integer, actual: " +
                  readableMap.apply(data), x)));
    } else {
      tryLength = data.get("length")
          .toTry()
          .mapTry(i -> Try.of(() -> Integer.parseInt(i))
              .getOrElseThrow(() -> new IllegalArgumentException(
                                                                 "Generate random string failed: length must be an integer. => e.g. length: 16 => actual: " +
                                                                     readableMap.apply(data))));
      ;
    }
